...
function processTrainInternal(train_info, i, j){
	...
	if(i === stations.length - 1) {
		...
		if(from != null && to != null) {
			var result = findStationStation(train_code, from, to);
			if(result == null) {
				queryTicketPrice(from, to, function(all_price) {
					if(all_price == null) {
						...
						insertIntoDb(train_code, train_info.sameTrains, stations[i], stations[j], seat_price, function(){
...
function findStationStation(train_code, from, to) {
	var key = train_code + ':' + from + '_' + to;
	return station_station_map[key];
}
	...
	if(seat_price_cache[key] == null) {
		...
		https.getHttpsData(url, key, {
			...
			try {
				...
				// JSON.parse() may throw exception
				logError('parse JSON object error: ' + ex);
			}
...
function insertIntoDb(train_code, same_trains, from_station, to_station, seat_price, cb) {
